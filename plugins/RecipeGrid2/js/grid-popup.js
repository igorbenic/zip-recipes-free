...
(function () {
    ...
    tinymce.create('tinymce.plugins.zrdnEditRecipeGrid', {
        init: function (editor, url) {
            ...
            editor.addButton('zrdn_buttons_grid', {
                title: 'Zip Recipes Grid',
                icon: 'zip-icon dashicons-screenoptions',
                onclick: function () {
                    // var recipe_id = null;
                    var grid = editor.dom.select('img.zrdn-grid')[0];
                    if (grid) {
                        editor.selection.select(grid);
                    ...
                    editor.insertContent('[zrdn-grid]');
                    // var iframe_url = baseurl + '/wp-admin/media-upload.php?post_id='+post_id + '&type=z_recipe_grid&tab=amd_zlrecipe&TB_iframe=true&width=640&height=523';
        ...
        _convert_codes_to_imgs: function (co) {
            return co.replace(/\[zrdn-grid\]/g, function (a, b) {
                return '<img id="zrdn-grid" class="zrdn-grid" src="' + plugindir + '/plugins/RecipeGrid2/images/iframe-placeholder.png" alt="" />';
            });
        },
        _convert_imgs_to_codes: function (co) {
            return co.replace(/\<img[^>]*?\sid="zrdn-grid.*?\>/g, function (a, b) {
                return '[zrdn-grid]';
            });
        },
        getInfo: function () {
            return {
                longname: "Zip Recipes Grid",
    ...
    tinymce.PluginManager.add('zrdn_plugin_grid', tinymce.plugins.zrdnEditRecipeGrid);
