...
function initTour(){
	for (var key in steps) {
		if (steps.hasOwnProperty(key)) {
			...
			if ($(step.attach).length) {
				if (key == 0) {
					prevLink[0] = zrdn_tour.configure_link;
					prevText[0] = zrdn_tour.configure_text;
				} else {
					prevLink[key] = steps[key - 1].link;
					prevText[key] = zrdn_tour.backBtnText;
				}

				if (key < stepCount - 1) {
					nextLink[key] = steps[parseInt(key) + 1].link;
					nextText[key] = zrdn_tour.nextBtnText;
				} else {
					nextLink[key] = nextLink[key-1];
					nextText[key] = zrdn_tour.endTour;
				}
				...
				tour.addStep(key, {
					...
					title: step.title,
					text: zrdn_tour.html.replace('{content}', step.text),
					buttons: [
						{
							text: prevText[key],
	...
	function cancel_tour() {
		...
		$.ajax({
			type: "POST",
			url: zrdn_tour.ajaxurl,
			dataType: 'json',
			data: ({
				action: 'zrdn_cancel_tour',
				token: zrdn_tour.token,
			})
		});
	};
